...
class VTONModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast(): 
            ...
            # TODO sd 1.5 unet_garm 提取服装特征, 尝试不传prompt_embeds的训练效果和传入空的prompt_embeds训练效果.encoder_hidden_states是必须要传的
    ...
    def log_images(self, batch, **kwargs):
        ...
        img_vton_mask = batch['img_vton_mask'].to(self.device)
        ...
        # print("-------------------------------------------_encode_prompt empty_embeds.shape111111 = ", empty_embeds.shape)
        # 预处理图片
        image_garm = self.image_processor.preprocess(image_garm)
        image_vton = self.image_processor.preprocess(image_vton)
        image_ori = self.image_processor.preprocess(image_ori) # TODO check这个做了mask没
    
        print("#########################image_vton tensor range2222",image_vton.min(), image_vton.max())
