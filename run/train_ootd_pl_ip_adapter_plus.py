...
from ip_adapter.ip_adapter import Resampler, IPAdapter
...
num_tokens = 4
image_proj_model = Resampler(
    dim=unet_vton.config.cross_attention_dim,
    depth=4,
    dim_head=64,
    heads=12,
    num_queries=num_tokens,
    embedding_dim=ip_image_encoder.config.hidden_size,
    output_dim=unet_vton.config.cross_attention_dim,
    ff_mult=4
)
...
if "latents" in state_dict["image_proj"] and "latents" in image_proj_model.state_dict():
    print("=====================latents in state_dict")
    # Check if the shapes are mismatched
    if state_dict["image_proj"]["latents"].shape != image_proj_model.state_dict()["latents"].shape:
...
print("===================================strict_load_image_proj_model = ", strict_load_image_proj_model)
image_proj_model.load_state_dict(state_dict["image_proj"], strict=strict_load_image_proj_model)
