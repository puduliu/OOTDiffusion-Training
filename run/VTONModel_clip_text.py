...
class VTONModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_type == 'hd':
            ...
            # TODO unet_vton和unet_garm的prompt_embeds应该要不一样，试试不一样的效果
            # prompt_embeds[:, 1:] = prompt_image[:] # TODO torch.Size([2, 77, 768]), shape没有改变，但是替换了prompt_image
    ...
    def log_images(self, batch, **kwargs):
        ...
        if self.model_type == 'hd':
            ...
            # prompt_embeds[:, 1:] = prompt_image[:] # TODO都注释掉，试试不用图像编码器，冻结unet_garm
        elif self.model_type == 'dc':
            prompt_embeds = self.text_encoder(self.tokenize_captions(prompt).to(self.device))[0]
            # prompt_embeds = torch.cat([prompt_embeds, prompt_image], dim=1)
        else:
