...
class FrozenDinoV2Encoder(AbstractEncoder):
    ...
    def __init__(self, device="cuda", freeze=True):
        ...
        self.projector = nn.Linear(1536,768) # TODO 这边加了个线性层?
    ...
    def forward(self, image): # TODO check attention map哪里可以输出
        ...
        # Anydoor已经确认，tensor范围是[0,1]
        print("===============================FrozenDinoV2Encoder1111 encode Min:", image.min().item(), "Max:", image.max().item())
        # todo FrozenDinoV2Encoder 
        if isinstance(image,list): # 把 list 中的 tensor 拼成一个 batch
            image = torch.cat(image,0)

        # TODO 根据这个归一化，大概率image是 tensor类型, 范围[0,1]
        image = (image.to(self.device)  - self.image_mean.to(self.device)) / self.image_std.to(self.device)
        print("===============================FrozenDinoV2Encoder2222 encode Min:", image.min().item(), "Max:", image.max().item())
        # -2.1007792949676514 Max: 2.552854299545288
        features = self.model.forward_features(image)
        # 假设 features 是一个字典，里面包含 NumPy 数组或 Tensor

        # print("------------------------------------features = ", features)
        tokens = features["x_norm_patchtokens"] #([1, 256, 1536])
        # print("------------------------------------tokens.shape = ", tokens.shape)
        image_features  = features["x_norm_clstoken"] #([1, 1536])
        ...
        hint = torch.cat([image_features,tokens],1) # 8,257,1024 hint.shape1 =  torch.Size([1, 257, 1536])
